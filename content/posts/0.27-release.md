...
Pyodide v0.27.0 is out. This release focuses on improving the long-term
stability of Pyodide.

# Welcome Agriya Khetarpal to the Pyodide team

Agriya Khetarpal has joined Pyodide as the new maintainer. Agriya has been
active in the Scientific Python area and is a contributor of NumPy, SciPy, and
scikit-learn. He has already contributed significantly to strengthening Pyodide
support in various Scientific Python packages.
...
more quickly, and you can apply these improvements to your packages sooner.
...
Recent versions of `pyodide-build` work with Pyodide 0.26 and higher. This means
you can use, say, `pyodide-build` version 0.29.2, to set up an older Pyodide
0.26.4 cross-build environment:
...
In case we decide to drop compatibility for a particular version of Pyodide, we
shall mark it as such in the 
[cross-build environments metadata file](https://github.com/pyodide/pyodide/blob/main/pyodide-cross-build-environments.json).

### Wheels for new packages

This release includes about a dozen new packages, most notably PyArrow, Polars,
and DuckDB, which users have long requested. What's unique about these new
...
Our next release should use Python 3.13. In addition, we are planning to make
...
We are working on restoring Emscripten to a tier 3 supported target for Python.
As a part of this work, we are upstreaming a large number of fixes to the
Emscripten file system. For example, it now works to seek on `/dev/null` (it
...
made their way into Pyodide yet because we're still using Emscripten 3.1.68 for
ABI compatibility.

### Wasm Exception Handling

We plan to switch from the legacy JS based exception handling to
using Wasm Exception Handling for C++ exceptions, Rust panics, and
setjmp/longjmp. This will lead to faster, smaller code, and fewer bugs. It also
eliminates a lot of complexity in the stack switching support code -- it is
impossible to stack switch through JavaScript frames and by default every C++
try block introduces a JavaScript frame. This requires upstream work on the Rust
compiler and switching to a custom build of the Rust standard library.
